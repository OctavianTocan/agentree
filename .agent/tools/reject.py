...
def main():
    ...
    p.add_argument("--reason", required=True, help="Why it's being rejected. Required.")
    p.add_argument("--reviewer", default="host-agent")
    args = p.parse_args()

    try:
        rejected = mark_rejected(args.candidate_id, args.reviewer, args.reason, CANDIDATES)
    except FileNotFoundError as e:
        print(f"ERROR: {e}", file=sys.stderr)
        sys.exit(1)

    print(f"rejected {args.candidate_id} (rejection_count={rejected.get('rejection_count', 1)})")
