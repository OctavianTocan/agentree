...
def index(pdf_path: str) -> list[TreeStructure]:
    ...
    # --- DOCUMENT INDEXING WITHOUT TABLE OF CONTENTS --- #
        ...
        document_structure = asyncio.run(
            generate_toc_initial_structure(chunk_pages[0].content)
        )
        logger.info(
            "initial_structure:\n%s",
        ...
        for chunk in chunk_pages[1:]:
            ...
            logger.info(
                ...
                TreeStructureList(sections=continuation_structure).model_dump_json(
                    indent=2
                ),
            )
...
def chunk_pages_with_overlap(
    pages: list[Page], overlap_page: int = 1
) -> list[PageChunk]:
    ...
    for page_index, page in enumerate(pages):
        ...
        if current_token_count + current_page_tokens > chunk_target:
            ...
            current_chunk_pages = [
                page.content for page in pages[overlap_start:page_index]
            ]
            current_token_count = sum(
                [page.tokens for page in pages[overlap_start:page_index]]
            )
