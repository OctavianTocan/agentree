...
def live_claude_tests_enabled() -> bool:
  ...
  return (
    os.getenv('AGENTREE_RUN_LIVE_TESTS') == '1' and settings.claude_code_oauth_token is not None
  )


requires_live_claude = pytest.mark.skipif(
  not live_claude_tests_enabled(),
  reason='live Claude tests require AGENTREE_RUN_LIVE_TESTS=1 and an OAuth token',
)
